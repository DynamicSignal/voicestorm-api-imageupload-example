...
function voicestormApiRequest($requestType, $url, $requestData = null)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (($requestData !== null) && array_key_exists('image', $requestData))
            {
                $file = $requestData['image'];
                $data = file_get_contents($file['tmp_name']);
                curl_setopt($ch, CURLOPT_BINARYTRANSFER, TRUE); 
                array_push($header, 'Content-Type:'.$file['type']);
            }
            else if(($requestData !== null) && ($requestType == "PUT" || $requestType == "POST"))
            {
